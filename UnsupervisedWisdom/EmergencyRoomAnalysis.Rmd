...
```{r, include=FALSE}
#cleanup
rm(gender, gentable, genderlables, out, percentoffemales)
...
```
```{r, include=FALSE}
rm(df_diagnosisfreq, df_diagnosisfreqSorted)
rm(highest4diagnosis)
rm(i, n, perc, percentoftop4diagnosis, sColor, sDiag, vec, vecColor, vecS, freq, row)
```
...
ggplot (injuriesbydate, aes (x = treatment_date, y = frequency, group = diagnosis, color = diagnosis)) +
  geom_line () +
  labs (title = "Frequency of Injury By Day",x = "Treatment Date", y = "Injury Frequency", color = "Diagnosis")
...
kable(locationresults, format = "html", align = "c", caption = "Combination of All Major Injury Factors")
...
I don't believe the case number or the treatment date will help the modeling, so I'll remove this column from df_modelingdata. For the modeling, the narrative column isn't not helpful.
...
For this model, I was initially thinking of using a hierarchical clustering model, but I believe my data exploration exposed a hierarchy, so I'm going to look at the pam clustering model to look at the data from a different perspective. Its quick and easy to do, but might reveal something interesting. Unfortunately the function has an upper limit of 65K rows. To work around this, I'll split the dataset into two small sets. The first is covers the range of 2019-2021, the second 2022. If you recall from the injuries based on time plot, we saw a significant increase in the daily injuries for that part of the dataset, so this gives us a chance to explore this question.

## Data Split
```{r}

#Dataset from 2019 - 2021
df2019 <- sqldf('SELECT *
                  FROM df_modelingdata
...
df2022 <- sqldf('SELECT *
                  FROM df_modelingdata
                  WHERE treatment_date > \'2021-12-31\'
                  ')

df2019 <- select(df2019, -c(treatment_date))
df2022 <- select(df2022, -c(treatment_date))
...
## PAM Model 2019-2021
For the first model, I'm taking the first 3 years of the dataset. This set has a lower median injuries than the 2022 set.
```{r}
...
if (system.file(package = "NbClust") == "") {
...
clusters <- 4 #selecting 4 because of the injuries

pam_result <- pam(df2019, clusters)
gc()#return memory
...
silResult <- silhouette(pam_result)

#plot(silResult)
fviz_silhouette(silResult)
print(silResult)

#bind the results of the model to the data
df2019Cluster <- cbind(df2019, pam_result$clustering)
head(df2019Cluster)
tail(df2019Cluster)
...
```{r}
colnames(df2019Cluster)[7] <- "cluster" 

clusteringresults1 <- sqldf('select cluster, diagnosis, sex, location, body_part, frequency
                        from(
                            SELECT sex, location, diagnosis, body_part, cluster, count(*) AS "frequency"
                            FROM df2019Cluster
                            GROUP BY sex, location, diagnosis, body_part
                            ORDER BY sex, location, diagnosis, body_part
                        ) as t
                        WHERE frequency > 1
                        ORDER BY cluster, frequency DESC
                        ')

mosaicplot(xtabs(frequency ~ sex + cluster + diagnosis + location +  body_part, data = clusteringresults1),
           main = "Combination of All Major Injury Factors for 2019-2021",
           shade = TRUE)
```
What this shows is 4 closely sized clusters, which a wide distribution of the locations, injuries, and body parts.The difference between the sexes, is consistent with what was seen in the data exploration.




## PAM Model 2
This model is going to be used for exploring the 2022, which represents a higher daily median number of injuries, and is worth looking at separtely.
...
pam_result2 <- pam(df2022, clusters)
gc()#return memory

# Print the cluster medoids
print(pam_result2$medoids)

# Generate a silhouette plot
silResult2 <- silhouette(pam_result2)

#plot(silResult2)
fviz_silhouette(silResult2)
print(silResult2)

#bind the results of the model to the data
df2022Cluster <- cbind(df2022, pam_result2$clustering)
head(df2022Cluster)
tail(df2022Cluster)

```
Let's look at how the clustering compares with data exploration.

```{r}

colnames(df2022Cluster)[7] <- "cluster" 

clusteringresults2 <- sqldf('select cluster, diagnosis, sex, location, body_part, frequency
                        from(
                            SELECT sex, location, diagnosis, body_part, cluster, count(*) AS "frequency"
                            FROM df2022Cluster
                            GROUP BY sex, location, diagnosis, body_part
...
mosaicplot(xtabs(frequency ~ sex + cluster + diagnosis + location +  body_part, data = clusteringresults2),
           main = "Combination of All Major Injury Factors for 2022",
           shade = TRUE)
```
The output from the 2022 PAM model is much different from the previous model in that cluster 1 contains the vast majority of the cases. It contains the full gambit of injuries and locations, while clusters 2-4 tend to favor the contusions and lacerations.
