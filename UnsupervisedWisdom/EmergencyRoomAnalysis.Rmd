...
Based on what we've learned through the data exploration, I believe we can focus on the top 4 diagnosed injuries: fractures, internal injuries, contusions, and lacerations. These 4 diagnoses comprise 86% of the dataset.
...
```
The new row count matches what was shown in the data exploration. So, this step is correct.
...
```{r}
library(dplyr)
df_modelingdata <- select(df_modelingdata, -c(race, hispanic, fire_involvement, alcohol, drug))
...
I don't believe the case number will help the modeling, so I'll remove this column from df_modelingdata.

```{r}
df_modelingdata <- select(df_modelingdata, -c(cpsc_case_number))
```
